...
def generate_player_slug(full_name):
    ...
    first_name, last_name = names[0], names[-1]  # Assumes the last word is the last name
    ...
    player_slug = f"{last_name_part}{first_name_part}01".lower()  # Convert to lowercase as in the example URL
...
def add_stats(df):
    ...
    for index, row in tqdm.tqdm(df.iterrows(), total=df.shape[0]):
        ...
        player_slug = generate_player_slug(player_name)
        
        # Construct the URL
        url = f"https://www.basketball-reference.com/players/n/{player_slug}/gamelog/2024"
        ...
        if response.status_code == 200:
            html_content = response.text
            soup = BeautifulSoup(html_content, 'html.parser')
            game_rows = [r for r in soup.findAll('tr') if r.find('td', {'data-stat': 'trb'})]
            game_stats = []
            ...
            ou_value = row['O/U']
            ou_value = row['O/U']
            df.at[index, 'Season Over Covered %'] = calculate_over_percentage(ou_value, game_stats)
            df.at[index, 'Last 10 Games Over Covered %'] = calculate_over_percentage(ou_value, game_stats[-10:])
            df.at[index, 'Last 5 Games Over Covered %'] = calculate_over_percentage(ou_value, game_stats[-5:])
